...
from reader import RoutinesDataset, INTERACTIVE, get_cooccurence_frequency, get_spectral_components, CombinedDataSplits
...
def run_model(data, group, checkpoint_dir=None, read_ckpt=False, write_ckpt=False, tags=[], logs_dir='logs', pretrain_data=[]):
    ...
    # logs_dir_5days = '/'.join(logs_dir.split('/'))[:-1]+'/5'
    # checkpoint_dir = os.path.join(logs_dir, group,'ourspt_0epochs')
    ...
    pretrain_loader = DataLoader(CombinedDataSplits(pretrain_data), num_workers=8, batch_size=data.params['batch_size'], sampler=data.train.sampler(), collate_fn=data.train.collate_fn)
    trainer = Trainer(gpus = torch.cuda.device_count(), max_epochs=30, logger=wandb_logger, log_every_n_steps=5, callbacks=[ckpt_callback])
    trainer.fit(model, pretrain_loader)
...
def run(data_dir, cfg = {}, baselines=False, ckpt_dir=None, read_ckpt=False, write_ckpt=False, tags=[], train_days=None, logs_dir='logs', pretrain_dirs=[]):
    ...
    def get_train_set(data_dir_pt):
        data = RoutinesDataset(data_path=os.path.join(data_dir_pt,'processed'), 
                           time_encoder=time_encoding, 
                           batch_size=cfg['BATCH_SIZE'],
                           only_seen_edges = cfg['ONLY_SEEN_EDGES'])
        return data.train

    pretrain_data = [get_train_set(dir) for dir in pretrain_dirs]
    ...
    if baselines:
        ...
        run_model(data, group=group, read_ckpt=read_ckpt, write_ckpt=write_ckpt, tags=tags, logs_dir=logs_dir, pretrain_data=pretrain_data)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_dirs', type=str, default='', help='Path to store putputs.')
    ...
    pretrain_dirs = [l.strip() for l in args.pretrain_dirs.split(',')]

    run(data_dir=args.path, cfg=cfg, baselines=args.baselines, read_ckpt=args.read_ckpt, write_ckpt=args.write_ckpt, tags = tags, train_days=args.train_days, logs_dir=args.logs_dir, pretrain_dirs=pretrain_dirs)