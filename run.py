...
from graph_visualizations import visualize_conditional_datapoint, visualize_unconditional_datapoint
...
def run_model(data, group, checkpoint_dir=None, read_ckpt=False, write_ckpt=False, tags=[], logs_dir='logs'):
    ...
    if read_ckpt:
        ...
        model = GraphTranslatorModule.load_from_checkpoint(checkpoint_file, 
                                                            ...
                                                            use_state_info=cfg['USE_STATE_INFO'],
                                                            feature_option_num=data.common_data['feature_options_num'],
        ...
        for epoch in epochs:
            if output_dir[-2] == '_':
                output_dir_new = output_dir[:-2]+'_'+'{:02d}'.format(epoch)+'epochs'+output_dir[-2:]
            else:
                output_dir_new = output_dir+'_'+'{:02d}'.format(epoch)+'epochs'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs_dir', type=str, default='logs_default', help='Path to store outputs.')
